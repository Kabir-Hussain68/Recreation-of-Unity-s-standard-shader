...
#include "AutoLight.cginc"
    ...
    #define FOG_ON 1
        ...
        #define ADDITIONAL_MASKED_DIRECTIONAL_SHADOWS_1
    #endif
#endif
...
struct InterpolatorVertex
{
    ...
    #if defined(LIGHTMAP_ON) || ADDITIONAL_MASKED_DIRECTIONAL_SHADOWS_1
...
float GetMetallic(Interpolator i)
{
...
float GetSmoothness(Interpolator i)
{
    float smoothness = 1;
...
float3 GetEmission(Interpolator i)
{
...
float GetOcclusion(Interpolator i)
{
...
float GetDetailMask(Interpolator i)
{
...
float3 GetAlbedo(Interpolator i)
{
...
void ComputeVertexLightColor (inout Interpolator i)
{
    #if defined(VERTEXLIGHT_ON)
...
float FadeShadows(Interpolator i, float attenuation)
{
    #if HANDLE_SHADOWS_BLENDING_IN_GI || ADDITIONAL_MASKED_DIRECTIONAL_SHADOWS_1
        // Unity light attenuation dosent fade shadows for us
...
void ApplySubtractiveLighting(Interpolator i, inout UnityIndirect indirectLight)
{
...
InterpolatorVertex VertexProgram(VertexData v)
{
    InterpolatorVertex i; 
    UNITY_INITIALIZE_OUTPUT(InterpolatorVertex, i);
...
UnityIndirect CreateIndirectLight (Interpolator i, float3 viewDir)
{
    ...
    indirectLight.specular = 0;
        ...
        float3 prob0 = Unity_GlossyEnvironment(
        ...
        envData.reflUVW = BoxProjection(
            ...
            {
                float3 prob1 = Unity_GlossyEnvironment(
                    UNITY_PASS_TEXCUBE_SAMPLER(unity_SpecCube1, unity_SpecCube0),
                    unity_SpecCube1_HDR, envData);
                indirectLight.specular = lerp(prob1, prob0, interpolator);   
            }
            else
            {
                indirectLight.specular = prob0;
            }
        #else
            indirectLight.specular = prob0;
        #endif
...
float3 GetTangentSpaceNormal(Interpolator i)
{
...
float4 ApplyFog(float4 color, Interpolator i)
{
    #if FOG_ON
...
void ApplyParallax(inout Interpolator i)
{
    #if defined(_PARALLAX_MAP)
...
FragmentOutput FragmentProgram(Interpolator i)
{
    ...
    FragmentOutput output;
        ...
        output.gBuffer3 = color;
            ...
            #endif
            output.gBuffer4 =
                UnityGetRawBakedOcclusion(shadowUV, i.worldPos.xyz);
        #endif
    #else
